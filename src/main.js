...
重要な指示:
1. 全ての項目において、調査で実際に参考にしたWebサイトがあれば、そのリンクを1つ以上 \`links\` 配列に含めて出力してください。
2. URLは必ずGoogle検索結果から得られた正確なものを使用し、**絶対に架空のURLや推測したURL（例: https://example.com/... など）を出力しないでください**。実在が確認できない場合は空配列 \`[]\` にしてください。
`
...
2. 地震情報 (title: "地震情報")
   - 開催場所の市区町村や近隣における ${dateStr} と現在における地震情報を調べる
...
1. 大規模地震 (title: "大規模地震情報")
   - ${dateStr} 前後に日本国内で発生した、または現在における震度5以上の地震情報を調べる
